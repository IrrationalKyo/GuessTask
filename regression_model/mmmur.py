...
def makeFilenames(data_name, cell_size, epoch, batch_size, timesteps, offset):
    ...
    id = "c"+ str(cell_size) + "_e" + str(epoch) + "_b" + str(batch_size) + "_ti" + str(timesteps) + "_o" + str(offset) + "l6"
...
def manualVerification(model, X, Y, batchSize):
    ...
    for l in range(label_card):
        ...
        print("mean deviation:" + str(totalDeviation/ len(pred_y)))
...
if __name__ == "__main__":
    ...
    for fileName in fileNames:
        ...
        for t in range(label_card):
            ...
            example = cvt.organizeTraceManyMultManyUni(
        ...
        train_Y = np.reshape(train_Y, (total_len, timesteps, 1))
        ...
        test_Y = np.reshape(test_Y, (total_len, timesteps, 1))
        ...
        if modelExists:
            ...
            model = gue.create_model_many_many(cell_size, (timesteps, label_card*2), True,
                                 batchSize,
                                 output_dim=1,
                                 timesteps=timesteps,
                                 loss="mse",
            ...
            for i in range(1):
                model.fit(train_X, train_Y, epochs=epoch, batch_size=batchSize,
                          verbose=1, validation_split=validation_ratio)
                model.reset_states()

...
'''
def list_to_example_sequence_binary(trace_list_old, label_card, offset=0, pred_len=1, seed=0):
    ...
    for i in range(sequence_len):
        ...
        example_datum[0] = int(trace_list[i])
        list_of_examples.append(example_datum)

        label_datum = np.zeros(pred_len*2)

        for j in range(pred_len):
            task_num = trace_list[i + offset + j + 1]
            label_datum[j * 2+ int(task_num)] = 1
